...
version: 3.7
description: "Skill创建与质量门控，含53项10层审计模型。触发词：创建skill、skill创建、skill审计、质量审计。不适用：skill发布、生态体检（用mu-skill-auditor）"
...
3. **审计脚本+清单 > 人工目测**——自动化扫描+53项10层逐项确认。WHY：目测=遗漏，脚本=可重复。对抗**规则遗忘+规则冲突**（失效1+2），跨章节一致性只能靠脚本校验。
...
- **Gotchas section**：环境里反直觉事实集中写，单位token价值高；**但Gotchas是"速查结论"不是"事故日志"——单条超3-5行的现象/根因/修复完整叙述、或条目>10条/占全文>40%，必须按分层处置(AP-34/L1-8)：①删除(结论已被IRON LAW/工作流覆盖)、②上浮固化("该怎么做"的操作结论→提炼一句进对应阶段步骤)、③下沉("某模板/环境的实现坑"→移到references/troubleshooting.md，主文件只留"症状→查阅"索引表)。判据:常规执行时需读它=上浮，只在改特定东西时才需=下沉，别处已说=删
...
> 失效模式→AP: 遗忘→3/5/8/17/25 | 冲突→4/14/16/18/27 | 膨胀→1/9/13/15/20/31/33/34 | 跨模式→6/7/10/11/12/19/23/24/26/28/29/30/32
...
**L10内容质量**: 👤L10-1 可验证性(yes/no) | 🔍L10-2 AP清零(无AP-1~34) | 👤L10-3 文案(无错别字) | 👤L10-4 已知局限 | 👤L10-5 停滞检测(stale_count) | 👤L10-6 边缘输入覆盖(极短/极长/空输入/非预期语言等边界条件是否有处理指引或在已知局限中声明)

## §9 已知局限

1. 53项10层模型覆盖文档/代码/安全/健壮性，不能覆盖设计品味——审计全绿≠产出优秀
...
| 文件 | 说明 |
|------|------|
| [quality-gates.md](references/quality-gates.md) | AP-1~34完整说明+10层审计模型+Eval+触发词优化 |
