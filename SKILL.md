...
3. **全面审计 > 目测或单项核验**——56项10层逐项确认，脚本核验与内容核验共同构成一次不可拆分的全面审计。WHY：目测会遗漏，脚本可重复但覆盖有限；任何单一检查都不能验证跨章节语义和执行质量。对抗**规则遗忘+规则冲突**（失效1+2）。
...
> **自动触发**：新建Skill→完整流程(阶段1→8) | 修改核心流程、规则库、引用架构或安全边界→全面审计（56项10层） | 发布前→全面审计+安全扫描
...
| 39 | 表格被空行截断 | 表格行间禁空行;跨表须空行+新表头 | v4.1.6 AP清单渲染裸管道符 | §1失效1 |

> 失效模式→AP: 遗忘→3/5/8/17/25/39 | 冲突→4/14/16/18/27/36 | 膨胀→1/9/13/15/20/31/33/34/37/38 | 跨模式→6/7/10/11/12/19/23/24/26/28/29/30/32 | 闭环缺失→35

## §8 全面审计：10层56项模型

全面审计是唯一的审计结论，必须逐项确认全部 56 项。`bash scripts/skill-audit.sh <skill-name>` 只执行其中可脚本核验的项目并提供证据，不产生“审计通过”的独立结论；脚本输出绿灯只能作为全面审计的一部分。支持环境变量 `SKILL_BASE` 指定 Skills 根目录（默认从脚本位置自动探测）。完整说明见 [references/quality-gates.md](references/quality-gates.md)。

| 层 | 审计目标 | 项数 | 脚本核验项 | 覆盖范围 |
|---|---|---|---|---|
| L1 | 文档结构 | 9 | 6 | frontmatter/desc/name/行数/refs索引/版本历史/Gotchas膨胀/表格连续性 |
...
**L10内容质量**: 👤L10-1 可验证性(yes/no) | 🔍L10-2 AP清零(无AP-1~39)+ICE闭环(标`ICE: required`时须有closure_check.*或等价质量门；静态检查存在性+shebang，执行验证由用户手动运行) | 👤L10-3 文案(无错别字) | 👤L10-4 已知局限(每条含三要素:能力边界→触发条件→降级路径;P/D/E分类标注;同类合并;≤3条;重要性降序;非硬伤不收入;AP-37) | 👤L10-5 停滞检测(stale_count) | 👤L10-6 边缘输入覆盖(极短/极长/空输入/非预期语言等边界条件是否有处理指引或在已知局限中声明)
...
| 文件 | 说明 |
|------|------|
| [quality-gates.md](references/quality-gates.md) | AP-1~39完整说明+10层审计模型+Eval+触发词优化 |
