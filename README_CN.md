...
> AI Agent 技能的质量守门员 —— 内置 55 项 10 层审计模型、38 条反模式与 8 阶段门控创建工作流。
...
| 场景 | 你说的话 | 实际效果 |
|------|---------|---------|
| 🆕 从零创建新技能 | "创建一个把 Markdown 转排版的 Skill" | 完整 8 阶段门控流程：需求 → 规划 → L1/L2/L3 编写 → 审计 → 交付 |
| 🔍 审计已有技能 | "帮我审计 my-skill 的质量" | 10 层 55 项逐项检查，自动脚本扫描 + 人工判断项 |
| 🚫 修复反模式 | "检查我的 Skill 有没有反模式" | 扫描 AP-1 至 AP-38 反模式目录，逐条标注根因与修复方案 |
...
10 层审计覆盖文档结构、架构一致性、代码质量、跨文件对齐、安全合规、健壮性和内容质量。29 项可通过脚本自动扫描，26 项需人工判断。每条规则都追溯至真实事故——不是抽象建议。
...
38 条反模式，每条追溯到真实事故，关联设计原则，配有具体修复方案。是实战验证的纠正措施，不是抽象建议。每次审计周期自动扫描。

v4.1 新增：AP-35（事故闭环 / ICE-5）、AP-36（附件文件职责重叠）、AP-37（已知局限膨胀——三要素结构 + P/D/E 分类 + ≤3 条 + 三维重要性排序）。

#### 自动化审计脚本

`skill-audit.sh` 一条命令批量扫描，检查 IRON LAW 位置、description 格式、安全风险、行数、代码质量、断链引用等——在每次交付前拦截问题。
...
硬性规则杜绝凭据泄露、人员数据暴露和受限系统访问。自动扫描在交付前拦截违规。`.skillignore` 确保用户态文件不进入版本控制。
...
|------|-----------------|---------|------------|--------------|
| 审计模型 | 55 项 10 层 | 无 | 无 | 不定 |
| 自动化扫描 | ✅ Shell 脚本 | ❌ | ❌ | 少见 |
| 反模式目录 | 38 条，追溯事故 | 无 | 无 | 较少 |
| 上下文感知加载 | ✅ 三层 L1/L2/L3 | ❌ 单文件 | ❌ | 不定 |
| 门控工作流 | ✅ 8 阶段入口/出口 | ❌ | ❌ | 少见 |
| 停滞检测 | ✅ 量化信号 | ❌ | ❌ | ❌ |
| 安全扫描 | ✅ 交付前 | 仅人工 | ❌ | 不定 |
| 触发词优化 | ✅ 10+10 测试，覆盖率检查 | ❌ | ❌ | ❌ |
| 已知局限标准 | ✅ 三要素 + P/D/E + ≤3 条 | ❌ | ❌ | ❌ |
...
| 审计已有技能 | 交付前质量检查 | "帮我审计 my-skill 的质量" |
...
| 交付 | 最终安全扫描 + 打包（发布动作委托专用工具） | "交付 my-skill" |
...
| 审计模型 | 10 层 55 项（29 项自动扫描，26 项人工判断） |
| 反模式 | 38 条，每条关联真实事故与修复方案 |
| 已知局限 | 三要素结构（能力边界→触发条件→降级路径）、P/D/E 分类、≤3 条、三维重要性评估 |
...
- **自动化安全扫描** —— `skill-audit.sh` 在每次交付前检查凭据泄露、PII 暴露和受限系统引用。
...
> AI Agent 技能的质量守门员 —— 55 项，10 层，38 条反模式，零妥协。
...
> **版本**：v4.1.5 · [在线主页](https://muippt.github.io/mu-skill-creator/) · [发布历史](https://github.com/muippt/mu-skill-creator/releases)
>
> **v4.1.5 更新**：阶段 8 改为交付门——止步于打包，发布动作委托专用工具，与声明范围自洽；审计模式字面量改用字符类（功能零变化）；ICE 闭环检查保持纯静态。
