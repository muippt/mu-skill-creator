...
> AI Agent 技能的质量守门员 —— 内置 55 项 10 层审计模型、37 条反模式与 8 阶段门控创建工作流。
...
| 🚫 修复反模式 | "检查我的 Skill 有没有反模式" | 扫描 AP-1 至 AP-37 反模式目录，逐条标注根因与修复方案 |
...
37 条反模式，每条追溯到真实事故，关联设计原则，配有具体修复方案。是实战验证的纠正措施，不是抽象建议。每次审计周期自动扫描。
...
> AI Agent 技能的质量守门员 —— 55 项，10 层，37 条反模式，零妥协。
...
>
> **v4.1 更新**：55 项审计模型（+2：L4-5 附件职责唯一、L10-4 已知局限规范）· 37 条反模式（+3：AP-35 ICE-5 事故闭环、AP-36 附件重叠、AP-37 已知局限膨胀）· 已知局限标准：三要素结构 + P/D/E 分类 + ≤3 条 + 三维重要性评估（不可逆性 / 发生概率 / 影响范围）
