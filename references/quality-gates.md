...
### AP-39：markdown 表格被空行截断
**症状**：表格中间出现一个空行，后续表格行紧跟着继续写。源文件里肉眼看仍是"一张表"，但 markdown 渲染器在空行处终止表格：空行后的行没有表头+分隔行，被当作普通段落渲染，用户看到的是带裸管道符 `|` 的文本（形似删除线或乱码）。
**后果**：所有支持 markdown 渲染的平台（ClawHub skill 卡片、GitHub 等渲染平台）都会渲染破碎；且这类问题在纯文本编辑/终端 review 时完全不可见，只有发布到渲染平台后才暴露。
**检查**：机检（skill-audit.sh L1-9）：扫描 markdown 文件中的表格行块（`|` 开头 `|` 结尾的连续行），若两个表格块之间仅隔一个空行、且新块首行之后没有表头分隔行（`|---|` 形式），报 ❌TBL截断+行号。
**修复**：二选一——①若本意是一张表：删除表格中间的空行，让所有行连续；②若本意是两张表：保留空行，但必须为新表补上完整表头行+分隔行（`| 列1 | 列2 |` + `|---|---|`）。禁止只补分隔行不补表头。
**根因事故**：mu-skill-creator v4.1.6，主文件 AP 清单在 AP-22 后混入一个空行，AP-23~38 共 16 行在 ClawHub 页面渲染为裸管道符文本，被用户误认为"奇怪的删除线"（v4.1.7 修复）。
**对应原则**：§1 失效1（规则遗忘——小改动引入空行未复查渲染）
