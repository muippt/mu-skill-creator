...
**后果**：intro 内容会随 Skill 加载进入 context，同时Skill市场管理更难
...
**修复**：删除整个 metadata 块；发布者信息由Skill市场系统自动记录，无需手动维护
...
**症状**：SKILL.md 正文里逐版本罗列 changelog（v1.0 做了什么、v1.1 修了什么……）
**后果**：版本历史是溯源信息不是执行指令，占行数挤占注意力预算却不参与任何生成/路由/决策，把有效指令挤出 context
**检查**：`grep -nE '^## .*(版本历史|更新记录|CHANGELOG|Changelog)' SKILL.md`，命中且其下有多条版本条目即触发
**修复**：SKILL.md 只留 1 行版本号 + `references/CHANGELOG.md` 链接（如 `当前版本 vX.X，完整历史见 references/CHANGELOG.md`），历史详情全部移到 CHANGELOG.md
**根因事故**：mu-visual-card v12.6 版本历史占 35 行 / 全文 615 行，迁移后省至 1 行
**对应**：§3 决策 1（三层模型控制 context 膨胀）

### AP-34：Gotchas/踩坑记录堆积膨胀
**症状**：Gotchas / 踩坑记录段从"反直觉事实速查"退化成"逐条事故日志"——每条都写完整的现象→根因→修复叙述，条目数 >10 或该段占全文 >40%
**后果**：它是 AP-33 的同构兄弟——都把"溯源/调试类信息"堆进 L2 主文件，占注意力预算却不参与生成/路由/决策，把有效指令挤出 context
**检查**：脚本 L1-8 自动统计踩坑段 `###` 条目数与占全文比例（>10 条或 >40% 告警）；同 L1-5 为信息告警不直接判红
**修复**：按分层处置——①删除（结论已被 IRON LAW/工作流覆盖 = AP-14 冗余）；②上浮固化（"该怎么做"的操作结论 → 提炼一句进对应阶段步骤）；③下沉（"某模板/环境的实现坑" → 移到 `references/troubleshooting.md`，主文件只留"症状→查阅"索引表）。判据一句话：常规执行时需读它 = 上浮，只在改特定东西时才需 = 下沉，别处已说 = 删
**根因事故**：某开源宣传类 Skill，27 条 Gotchas 占 389 行 / 全文 66%，主文件 593 行
**对应**：§3 决策 1（三层模型控制 context 膨胀）+ §1 失效 3（规则膨胀）
