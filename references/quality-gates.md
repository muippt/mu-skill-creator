...
**检查**：`grep -ciE 'ICE:\s*required|incident-closure:\s*required' SKILL.md` — 标记后检查 scripts/ 下是否有 closure_check.* 或等价质量门脚本；审计默认静态检查存在性，设 `AUDIT_RUN_CLOSURE=1` 时真实执行并验证退出 0
